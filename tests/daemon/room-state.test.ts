...
describe("room-state", () => {
	...
	it("tracks per-agent cooldowns", () => {
		const state = createRoomState();
		markReplySent(state, "!room:garden", "$root1", "planner", 10_000);
		expect(isAgentCoolingDown(state, "!room:garden", "planner", 11_000, 1500)).toBe(true);
		expect(isAgentCoolingDown(state, "!room:garden", "planner", 12_000, 1500)).toBe(false);
	});

	it("tracks total and per-agent budgets per root event", () => {
		const state = createRoomState();
		expect(canReplyForRoot(state, "!room:garden", "$root1", "planner", 2, 4)).toBe(true);
		markReplySent(state, "!room:garden", "$root1", "planner", 1_000);
		expect(canReplyForRoot(state, "!room:garden", "$root1", "planner", 2, 4)).toBe(true);
		markReplySent(state, "!room:garden", "$root1", "planner", 2_000);
		expect(canReplyForRoot(state, "!room:garden", "$root1", "planner", 2, 4)).toBe(false);
	});

	it("enforces total reply budget across agents for the same root", () => {
		const state = createRoomState();
		markReplySent(state, "!room:garden", "$root1", "host", 1_000);
		markReplySent(state, "!room:garden", "$root1", "planner", 2_000);
		markReplySent(state, "!room:garden", "$root1", "critic", 3_000);
		markReplySent(state, "!room:garden", "$root1", "host", 4_000);
		expect(canReplyForRoot(state, "!room:garden", "$root1", "planner", 2, 4)).toBe(false);
	});
	...
	it("prunes stale root reply and cooldown state", () => {
		...
		expect(canReplyForRoot(state, "!room:garden", "$root1", "planner", 2, 4)).toBe(true);
		expect(isAgentCoolingDown(state, "!room:garden", "planner", 2_500, 1500)).toBe(false);
		expect(canReplyForRoot(state, "!room:garden", "$root1", "planner", 2, 4)).toBe(true);
	});
});
