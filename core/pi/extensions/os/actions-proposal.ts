...
import { getGardenRepoDir } from "../../../lib/filesystem.js";
...
export async function handleNixConfigProposal(
	...
	const repoDir = getGardenRepoDir();
	if (!existsSync(repoDir)) {
		return errorResult(`Local Garden repo not found at ${repoDir}. The proposal workflow expects a cloned repo there.`);
	}
	...
	const text = [
		`Validated local Garden repo at ${repoDir}`,
		"",
