...
export const STEP_GUIDANCE: Record<StepName, string> = {
	...
	connectivity:
		"Summarize how to connect: (1) Locally at localhost if sitting at the device, (2) Via NetBird mesh IP from any peer device. Show the mesh IP from: netbird status. Mention SSH: ssh pi@<mesh-ip>. The default password is set in bib-config.toml (copied from bib-config.example.toml) during image build — remind the user they can change it with: passwd. Recommend they add their SSH key with ssh-copy-id for passwordless access.",
	webdav:
