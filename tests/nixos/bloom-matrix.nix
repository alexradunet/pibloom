# tests/nixos/bloom-matrix.nix
# Test that the Bloom Matrix homeserver (Conduwuity) starts and accepts connections
...
pkgs.testers.runNixOSTest {
  ...
  nodes.server = { ... }: {
    ...
    networking.hostName = "bloom-matrix-test";
    time.timeZone = "UTC";
  ...
  testScript = { nodes, ... }: ''
    ...
    server.succeed("test -f /etc/bloom/matrix.toml")
    config_content = server.succeed("cat /etc/bloom/matrix.toml")
    ...
    status = server.succeed("systemctl show bloom-matrix.service -p User")
    ...
    server.succeed("systemctl restart bloom-matrix.service")
    server.wait_for_unit("bloom-matrix.service", timeout=60)
    server.succeed("curl -sf http://localhost:6167/_matrix/client/versions")
    
    # Test 10: Service is in wantedBy multi-user.target
    server.succeed("systemctl list-dependencies multi-user.target | grep -q bloom-matrix")
    
    print("All bloom-matrix tests passed!")
  '';
}
