...
SSH_USER="${NIXPI_DESKTOP_INTERACTION_SSH_USER:-human}"
...
for _ in $(seq 1 90); do
    if ssh "${SSH_OPTS[@]}" "${SSH_USER}@localhost" "echo ready" >/dev/null 2>&1; then
        break
    fi
    sleep 2
done

ssh "${SSH_OPTS[@]}" "${SSH_USER}@localhost" "echo ready" >/dev/null

scp "${SCP_OPTS[@]}" "${ROOT_DIR}/tools/desktop-probe.sh" "${SSH_USER}@localhost:/tmp/desktop-probe.sh" >/dev/null
ssh "${SSH_OPTS[@]}" "${SSH_USER}@localhost" "chmod +x /tmp/desktop-probe.sh && /tmp/desktop-probe.sh >/tmp/nixpi-desktop-probe.stdout"
scp "${SCP_OPTS[@]}" "${SSH_USER}@localhost:/tmp/nixpi-desktop-probe/result.json" "${ARTIFACT_DIR}/result.json" >/dev/null
scp "${SCP_OPTS[@]}" "${SSH_USER}@localhost:/tmp/nixpi-desktop-probe/root.png" "${ARTIFACT_DIR}/root.png" >/dev/null
