...
LOCAL_FLAKE_REF="${LOCAL_FLAKE_DIR}#nixos"
STATUS_DIR="${NIXPI_PRIMARY_HOME}/.nixpi"
STATUS_FILE="$STATUS_DIR/update-status.json"
CHECKED=$(date -u +"%Y-%m-%dT%H:%M:%SZ")

if [[ ! -f "${LOCAL_FLAKE_DIR}/flake.nix" ]]; then
  echo "Missing ${LOCAL_FLAKE_DIR}/flake.nix; NixPI updates require the standard /etc/nixos flake that imports /srv/nixpi." >&2
  exit 1
fi
...
NEW_SYSTEM=$(nix build "${LOCAL_FLAKE_DIR}#nixosConfigurations.nixos.config.system.build.toplevel" --no-link --print-out-paths 2>/dev/null || echo "")
...
if [[ "$AVAILABLE" = "true" ]]; then
  if nixos-rebuild switch --flake "$LOCAL_FLAKE_REF"; then
