...
const DEFAULT_CHECK = "checks.x86_64-linux.bloom-config";
...
export async function handleNixConfigProposal(
	...
	const repoDir = getBloomRepoDir();
	if (!existsSync(repoDir)) {
		return errorResult(`Local Bloom repo not found at ${repoDir}. The proposal workflow expects a cloned repo there.`);
	}
	...
	const text = [
		`Validated local Bloom repo at ${repoDir}`,
		"",
